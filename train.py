...
from lib.util.general import prepare_sub_folder, write_loss, get_config, to_gpu
import lib.trainer
import os
import time
import argparse
...
def parse_args():
    ...
    parser.add_argument("-c", "--config", type=str, default="configs/trans1x.yaml", help="Path to the config file.")
...
def train_with_config(config, opts, logger=None):
    ...
    output_directory = os.path.join(opts.out_dir, config.name)
